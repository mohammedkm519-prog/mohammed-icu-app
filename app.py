...
try:
    ...
    print("⚠️ OpenAI غير متاحة")

# ضع مفتاح DeepSeek الخاص بك هنا
DEEPSEEK_API_KEY = "sk-991e10c8c73f"  # استبدل بالمفتاح الصحيح
DEEPSEEK_BASE_URL = "https://api.deepseek.com"

if DEEPSEEK_API_KEY and DEEPSEEK_API_KEY != "sk-...":
...
# ======================================================================
# 📸 تحليل الصور باستخدام Gemini و DeepSeek
# ======================================================================
...
def analyze_lab_image(image_file):
    """تحليل صورة التحاليل باستخدام Gemini و DeepSeek"""
    ...
    if GEMINI_AVAILABLE:
        try:
            ...
            test_response = test_model.generate_content("قل مرحباً")
            
            if test_response and test_response.text:
                prompt = """
                أنت نظام ذكاء اصطناعي متخصص في قراءة نتائج المختبرات الطبية.
                
                هذه صورة لورقة تحاليل طبية. قم بقراءة الأرقام من الصورة.
                
                ابحث عن هذه القيم في الصورة وأعدها كالتالي:
                ...
                إذا لم تجد قيمة معينة، اكتب "غير موجود".
                """
    ...
    # ===== المصدر 2: DeepSeek =====
    if DEEPSEEK_AVAILABLE and OPENAI_AVAILABLE:
        try:
    ...
    return {
        ...
        "message": "يرجى التحقق من الاتصال أو إدخال البيانات يدوياً"
    }
    ...
    if patient:
        ...
        with tabs[2]:
            ...
            if uploaded_lab:
                ...
                with col2:
                    ai_sources = []
                    if GEMINI_AVAILABLE:
                        ai_sources.append("Gemini")
                    if DEEPSEEK_AVAILABLE:
                        ai_sources.append("DeepSeek")
                    st.info(f"🤖 المصادر المتاحة: {', '.join(ai_sources)}")
        ...
        with tabs[4]:
            ...
            if not has_permission(st.session_state.user, "ai_analysis"):
                ...
                st.info("🤖 يستخدم Gemini أو DeepSeek للتحليل")
