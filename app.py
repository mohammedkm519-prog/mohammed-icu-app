...
# ======================================================================
# 🔑 Gemini API (مضاف في البداية لضمان عمله)
# ======================================================================
...
def analyze_lab_image(image_file):
    """تحليل صورة التحاليل - النسخة النهائية"""
    ...
    try:
        ...
        هذه صورة تحتوي على جدول تحاليل طبية (CBC - Complete Blood Count).
        ...
        # البحث عن WBC
        wbc_match = re.search(r'[Ww][Bb][Cc][\s:]+([\d.]+)', text)
        if wbc_match:
            numbers['wbc'] = float(wbc_match.group(1))
        
        # البحث عن HB
        hb_match = re.search(r'[Hh][Bb][\s:]+([\d.]+)', text)
        if hb_match:
            numbers['hb'] = float(hb_match.group(1))
        
        # البحث عن PLT
        plt_match = re.search(r'[Pp][Ll][Tt][\s:]+([\d.]+)', text)
        if plt_match:
            numbers['plt'] = float(plt_match.group(1))
        ...
        return {
            "success": False,
            "error": "❌ لم يتم العثور على البيانات في الصورة",
            "data": {},
            "raw_text": text
        }
...
def analyze_radiology_image(image_file):
    ...
    if not GEMINI_AVAILABLE:
        return {"success": False, "error": "Gemini API غير متاحة"}
