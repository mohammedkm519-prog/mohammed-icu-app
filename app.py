...
# ======================================================================
# الإصدار النهائي - مع جميع الميزات
# ======================================================================
...
AUTHORIZED_USERS = {
    ...
    "dr_ahmed": {
        ...
        "name": "د. أحمد",
        "approved": True,
        "permissions": ["view_patients", "ai_analysis", "radiology", "voice", "alerts"]
    },
    "dr_sara": {
...
if 'patients' not in st.session_state:
    st.session_state.patients = [
        ...
        },
        {
            ...
            "weight": 75,
            "height": 168,
            "chronic_diseases": ["فشل قلبي", "ارتفاع الضغط"],
            "allergies": ["الأسبرين"],
            "surgeries": ["مفتاح قلبي 2015"],
            "current_medications": ["الديجوكسين", "الفيوروسيمايد"],
        ...
        },
        {
            ...
            "weight": 58,
            "height": 162,
            "chronic_diseases": ["الغدة الدرقية"],
            "allergies": ["لا يوجد"],
            "surgeries": ["استئصال الغدة الدرقية 2021"],
            "current_medications": ["الليفوثيروكسين"],
...
def dashboard():
    ...
    if patient:
        ...
        with tabs[5]:
            ...
            if not has_permission(st.session_state.user, "radiology"):
                st.warning("⚠️ " + ("ليس لديك صلاحية" if lang == "العربية" else "You don't have permission" if lang == "English" else "Yetkiniz yok" if lang == "Türkçe" else "شما مجوز ندارید"))
            else:
                # رفع صورة
                uploaded_file = st.file_uploader(
                    L['upload_image'] + " (X-Ray, CT, MRI)", 
                    type=["jpg", "jpeg", "png", "dcm", "bmp"],
                ...
                if uploaded_file:
                    ...
                    with col2:
                        description = st.text_area("📝 " + ("وصف الصورة" if lang == "العربية" else "Image Description" if lang == "English" else "Resim Açıklaması" if lang == "Türkçe" else "توضیحات تصویر"))
                        
                        if st.button("💾 " + L['save_image'], use_container_width=True):
                            image_data = base64.b64encode(uploaded_file.read()).decode('utf-8')
                            st.success("✅ " + ("تم حفظ الصورة بنجاح!" if lang == "العربية" else "Image saved successfully!" if lang == "English" else "Resim başarıyla kaydedildi!" if lang == "Türkçe" else "تصویر با موفقیت ذخیره شد!"))
                            st.info(f"📊 {'نوع الصورة' if lang == 'العربية' else 'Image Type' if lang == 'English' else 'Resim Türü' if lang == 'Türkçe' else 'نوع تصویر'}: {uploaded_file.type}")
                            st.info(f"📏 {'الحجم' if lang == 'العربية' else 'Size' if lang == 'English' else 'Boyut' if lang == 'Türkçe' else 'سایز'}: {uploaded_file.size} bytes")
                
                st.markdown("---")
                st.subheader("📸 " + L['patient_images'])
                st.info("💡 " + ("سيتم عرض الصور المحفوظة هنا" if lang == "العربية" else "Saved images will appear here" if lang == "English" else "Kaydedilmiş resimler burada görünecek" if lang == "Türkçe" else "تصاویر ذخیره شده در اینجا نمایش داده می‌شوند"))
        
        # ===== تبويب 7: التسجيل الصوتي =====
        with tabs[6]:
            st.subheader(L['voice_record'])
            
            if not has_permission(st.session_state.user, "voice"):
                st.warning("⚠️ " + ("ليس لديك صلاحية" if lang == "العربية" else "You don't have permission" if lang == "English" else "Yetkiniz yok" if lang == "Türkçe" else "شما مجوز ندارید"))
            else:
                st.info("🎤 " + ("لتسجيل ملاحظة صوتية، اضغط على زر التسجيل أدناه" if lang == "العربية" else "To record a voice note, click the record button below" if lang == "English" else "Ses notu kaydetmek için aşağıdaki kayıt düğmesine tıklayın" if lang == "Türkçe" else "برای ضبط یادداشت صوتی، روی دکمه ضبط زیر کلیک کنید"))
                
                audio_file = st.file_uploader(
                    "🎵 " + ("رفع ملف صوتي" if lang == "العربية" else "Upload Audio File" if lang == "English" else "Ses Dosyası Yükle" if lang == "Türkçe" else "آپلود فایل صوتی"),
                    type=["wav", "mp3", "m4a", "ogg"],
                    key="audio_upload"
                )
                
                if audio_file:
                    st.success(f"✅ {'تم رفع الملف الصوتي بنجاح!' if lang == 'العربية' else 'Audio file uploaded successfully!' if lang == 'English' else 'Ses dosyası başarıyla yüklendi!' if lang == 'Türkçe' else 'فایل صوتی با موفقیت آپلود شد!'}")
                    st.info(f"📊 {'اسم الملف' if lang == 'العربية' else 'File Name' if lang == 'English' else 'Dosya Adı' if lang == 'Türkçe' else 'نام فایل'}: {audio_file.name}")
                    st.info(f"📏 {'الحجم' if lang == 'العربية' else 'Size' if lang == 'English' else 'Boyut' if lang == 'Türkçe' else 'سایز'}: {audio_file.size} bytes")
                    
                    notes = st.text_area("📝 " + ("ملاحظات إضافية" if lang == "العربية" else "Additional Notes" if lang == "English" else "Ek Notlar" if lang == "Türkçe" else "یادداشت‌های اضافی"))
                    
                    if st.button("💾 " + ("حفظ التسجيل" if lang == "العربية" else "Save Recording" if lang == "English" else "Kaydı Kaydet" if lang == "Türkçe" else "ذخیره ضبط"), use_container_width=True):
                        st.success("✅ " + ("تم حفظ التسجيل الصوتي بنجاح!" if lang == "العربية" else "Voice recording saved successfully!" if lang == "English" else "Ses kaydı başarıyla kaydedildi!" if lang == "Türkçe" else "ضبط صوتی با موفقیت ذخیره شد!"))
                
                st.markdown("---")
                st.subheader("📋 " + ("التسجيلات الصوتية السابقة" if lang == "العربية" else "Previous Voice Recordings" if lang == "English" else "Önceki Ses Kayıtları" if lang == "Türkçe" else "ضبط‌های صوتی قبلی"))
                st.info("💡 " + ("سيتم عرض التسجيلات الصوتية المحفوظة هنا" if lang == "العربية" else "Saved voice recordings will appear here" if lang == "English" else "Kaydedilmiş ses kayıtları burada görünecek" if lang == "Türkçe" else "ضبط‌های صوتی ذخیره شده در اینجا نمایش داده می‌شوند"))
        ...
        with tabs[7]:
            ...
            # إحصائيات المرضى
            st.markdown("### 📊 " + ("إحصائيات المرضى" if lang == "العربية" else "Patient Statistics" if lang == "English" else "Hasta İstatistikleri" if lang == "Türkçe" else "آمار بیماران"))
            
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("👨‍⚕️ " + ("مجموع المرضى" if lang == "العربية" else "Total Patients" if lang == "English" else "Toplam Hasta" if lang == "Türkçe" else "مجموع بیماران"), len(st.session_state.patients))
            with col2:
            ...
            with st.expander("📝 " + L['add_patient']):
                col1, col2 = st.columns(2)
                with col1:
                    new_name = st.text_input(L['full_name'])
                ...
                with col2:
                    new_diagnosis = st.text_input("📋 " + ("التشخيص" if lang == "العربية" else "Diagnosis" if lang == "English" else "Teşhis" if lang == "Türkçe" else "تشخیص"))
                    new_risk = st.selectbox("🟡 " + L['risk_level'], ["منخفض", "متوسط", "مرتفع", "حرج"])
                    new_chronic = st.text_input(L['chronic_diseases'], placeholder="مثال: السكري, ارتفاع الضغط")
                    new_allergies = st.text_input(L['allergies'], placeholder="مثال: البنسلين")
                    new_medications = st.text_input(L['current_medications'], placeholder="مثال: الأنسولين, الليسينوبريل")
                
                if st.button("➕ " + L['add_patient'], use_container_width=True, type="primary"):
                    if new_name and new_diagnosis:
                        new_patient = {
                            ...
                            "weight": new_weight,
                            "height": new_height,
                            "chronic_diseases": [d.strip() for d in new_chronic.split(",")] if new_chronic else [],
                            "allergies": [a.strip() for a in new_allergies.split(",")] if new_allergies else [],
                            "surgeries": [],
                            "current_medications": [m.strip() for m in new_medications.split(",")] if new_medications else [],
                        ...
                        st.success(f"✅ {'تم إضافة المريض ' + new_name + ' بنجاح!' if lang == 'العربية' else 'Patient ' + new_name + ' added successfully!' if lang == 'English' else 'Hasta ' + new_name + ' başarıyla eklendi!' if lang == 'Türkçe' else 'بیمار ' + new_name + ' با موفقیت اضافه شد!'}")
                        st.rerun()
                    else:
                        st.warning("⚠️ " + ("الرجاء إدخال الاسم والتشخيص" if lang == "العربية" else "Please enter name and diagnosis" if lang == "English" else "Lütfen ad ve teşhis girin" if lang == "Türkçe" else "لطفاً نام و تشخیص را وارد کنید"))
            
            # توزيع الأعمار
            st.markdown("---")
            st.markdown("### 📊 " + ("توزيع الأعمار" if lang == "العربية" else "Age Distribution" if lang == "English" else "Yaş Dağılımı" if lang == "Türkçe" else "توزیع سن"))
            ages = [p['age'] for p in st.session_state.patients]
            age_df = pd.DataFrame({
                "العمر" if lang == "العربية" else "Age" if lang == "English" else "Yaş" if lang == "Türkçe" else "سن": ages
            })
            st.dataframe(age_df, use_container_width=True)
            
            # توزيع الجنس
            st.markdown("### 📊 " + ("توزيع الجنس" if lang == "العربية" else "Gender Distribution" if lang == "English" else "Cinsiyet Dağılımı" if lang == "Türkçe" else "توزیع جنسیت"))
            genders = [p['gender'] for p in st.session_state.patients]
            gender_df = pd.DataFrame({
                "الجنس" if lang == "العربية" else "Gender" if lang == "English" else "Cinsiyet" if lang == "Türkçe" else "جنسیت": genders
            })
