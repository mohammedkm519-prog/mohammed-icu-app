...
try:
    ...
    print("✅ Tesseract OCR متاحة")
except:
    PYTESSERACT_AVAILABLE = False
    print("⚠️ Tesseract OCR غير متاحة")
...
# ======================================================================
# 📸 نظام OCR والذكاء الاصطناعي المتعدد المصادر
# ======================================================================
...
def extract_numbers_from_text(text):
    """استخراج الأرقام من النص باستخدام تعابير نمطية محسّنة"""
    numbers = {}
    ...
    # البحث عن الأرقام في النص مع تحسين الدقة
...
def analyze_lab_image_multi_source(image_file):
    """تحليل صورة التحاليل باستخدام مصادر متعددة مع OCR"""
    ...
    try:
        ...
        image.seek(0)  # إعادة تعيين المؤشر
    except:
        image = None
    
    extracted_text = ""
    
    # ===== المصدر 1: OCR باستخدام EasyOCR =====
    if EASYOCR_AVAILABLE and image:
        try:
            ...
            data = extract_numbers_from_text(extracted_text)
            if data:
                return {
                    "success": True,
                    "data": data,
                    "source": "EasyOCR + استخراج ذكي",
                    "raw_text": extracted_text
                }
        except Exception as e:
            print(f"⚠️ EasyOCR فشل: {e}")
    
    # ===== المصدر 2: OCR باستخدام Tesseract =====
        ...
        try:
            ...
            if data:
                return {
                    "success": True,
                    "data": data,
                    "source": "Tesseract OCR + استخراج ذكي",
                    "raw_text": extracted_text
                }
        except Exception as e:
            print(f"⚠️ Tesseract فشل: {e}")
    ...
    if GEMINI_AVAILABLE:
        try:
            ...
            استخرج جميع الأرقام والنتائج الموجودة في هذه الصورة بدقة.
    ...
    # ===== المصدر 4: إذا كان هناك نص مستخرج مسبقاً =====
    if extracted_text:
        data = extract_numbers_from_text(extracted_text)
        if data:
            return {
                "success": True,
                "data": data,
                "source": "OCR متعدد المصادر",
                "raw_text": extracted_text
            }
    
    # ===== المصدر 5: طلب إدخال يدوي =====
    return {
        "success": False,
        "error": "لم نتمكن من استخراج الأرقام من الصورة تلقائياً. يرجى إدخال البيانات يدوياً.",
        "data": {},
        "source": "جميع المصادر غير متاحة",
        "raw_text": ""
    }
...
def analyze_radiology_image_multi(image_file):
    ...
    return {
        "success": False,
        "error": "Gemini غير متاحة لتحليل الأشعة",
        "analysis": ""
    }
...
- ضغط الدم: {bp}/{patient['vitals']['diastolic_bp']} mmHg- معدل النبض: {hr} نبضة/دقيقة
- درجة الحرارة: {temp}°C
- SpO2: {spo2}%
    ...
    if patient:
        ...
        with tabs[2]:
            ...
            if uploaded_lab:
                ...
                with col2:
                    st.info("🤖 سيتم استخدام الذكاء الاصطناعي المتعدد المصادر مع OCR")
