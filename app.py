...
# ======================================================================
# 🔑 إعدادات الذكاء الاصطناعي المزدوج (Gemini + DeepSeek)
# ======================================================================
...
try:
    ...
    print("✅ DeepSeek API متاحة")
except:
    DEEPSEEK_AVAILABLE = False
    print("⚠️ DeepSeek غير متاحة")
...
def analyze_lab_image_with_gemini(image_file):
    """تحليل صورة التحاليل باستخدام Gemini"""
...
# ======================================================================
# 🤖 الذكاء الاصطناعي المزدوج (Gemini + DeepSeek)
# ======================================================================

def build_clinical_context(patient):
    """بناء السياق السريري للمريض"""
    context = f"""
    ===== معلومات المريض =====
    الاسم: {patient['name']}
    ...
    معدل النبض: {patient['vitals']['heart_rate']} نبضة/دقيقة
    درجة الحرارة: {patient['vitals']['temperature']}°C
    معدل التنفس: {patient['vitals']['respiratory_rate']} نفس/دقيقة
    SpO2: {patient['vitals']['spo2']}%
    CVP: {patient['vitals']['cvp']} mmHg
    ICP: {patient['vitals']['icp']} mmHg
    
    ===== نتائج المختبر =====
    WBC: {patient['labs']['wbc']} ×10³/µL
    HB: {patient['labs']['hb']} g/dL
    PLT: {patient['labs']['plt']} ×10³/µL
    pH: {patient['labs']['ph']}
    PaCO2: {patient['labs']['paco2']} mmHg
    PaO2: {patient['labs']['pao2']} mmHg
    HCO3: {patient['labs']['hco3']} mEq/L
    Na: {patient['labs']['na']} mEq/L
    K: {patient['labs']['k']} mEq/L
    Ca: {patient['labs']['ca']} mEq/L
    Creatinine: {patient['labs']['creatinine']} mg/dL
    BUN: {patient['labs']['bun']} mg/dL
    ALT: {patient['labs']['alt']} U/L
    AST: {patient['labs']['ast']} U/L
    Bilirubin: {patient['labs']['bilirubin']} mg/dL
    INR: {patient['labs']['inr']}
    Troponin: {patient['labs']['troponin']} ng/mL
    Lactate: {patient['labs']['lactate']} mmol/L
    
    ===== درجات التقييم =====
    SOFA Score: {patient['sofa_score']}
    NEWS2 Score: {patient['news2_score']}
    GCS: {patient['gcs_score']}
    مستوى الخطورة: {patient['risk_level']}
    ...
    الاحتياجات السائلة: {calculate_fluid_needs(patient['weight']):.0f} مل/يوم
    """
    return context

def get_ai_diagnosis_dual(patient):
    """تشخيص باستخدام Gemini أولاً، ثم DeepSeek، ثم المحاكاة"""
    ...
    if DEEPSEEK_AVAILABLE:
        try:
            ...
            context = build_clinical_context(patient)
            
            response = client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": "أنت طبيب متخصص في العناية المركزة. قم بتحليل بيانات المريض وتقديم تشخيص مفصل وخطة علاجية."},
    ...
    # ===== المصدر 3: المحاكاة الداخلية =====
    return {
        "success": True,
        "diagnosis": generate_advanced_diagnosis(patient),
        "source": "AI Simulator"
    }

def generate_advanced_diagnosis(patient):
    """توليد تشخيص محاكي متقدم"""
...
def dashboard():
    ...
    with st.sidebar:
        ...
        st.markdown("### 🤖 حالة الذكاء الاصطناعي")
        if GEMINI_AVAILABLE:
            st.success("✅ Gemini AI")
        else:
            st.error("❌ Gemini")
        if DEEPSEEK_AVAILABLE:
            st.success("✅ DeepSeek AI")
        else:
            st.error("❌ DeepSeek")
    ...
    if patient:
        ...
        with tabs[4]:
            ...
            if not has_permission(st.session_state.user, "ai_analysis"):
                ...
                st.info("🤖 يستخدم الذكاء الاصطناعي المزدوج: Gemini + DeepSeek")
                ...
                if st.button(L['ai_analysis'], use_container_width=True, type="primary"):
                    with st.spinner("🧠 " + ("جاري التحليل..." if lang == "العربية" else "Analyzing...")):
                        result = get_ai_diagnosis_dual(patient)
                        if result["success"]:
